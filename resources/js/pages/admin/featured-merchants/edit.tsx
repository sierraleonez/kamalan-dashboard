...
export default function FeaturedMerchantEdit({ featuredMerchant, merchants, errors = {} }: Props) {
    const today = new Date().toISOString().split('T')[0];

    const { data, setData, put, processing } = useForm({
        merchant_id: featuredMerchant.merchant_id.toString(),
        priority: featuredMerchant.priority.toString(),
        subscription_date_start: featuredMerchant.subscription_date_start,
        subscription_date_end: featuredMerchant.subscription_date_end,
    });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(admin.featuredMerchants.update(featuredMerchant.id).url);
    };

    return (
        <AppLayout title={`Edit Featured Merchant: ${featuredMerchant.merchant.name}`}>
            <form onSubmit={handleSubmit} className="space-y-6 max-w-lg mx-5 mt-8">
                ...
                <div className="flex justify-end gap-2">
                    ...
                    <Button type="submit" disabled={processing}>
                        {processing ? 'Updating...' : 'Update Featured Merchant'}
                    </Button>
                </div>
            </form>
        </AppLayout>
